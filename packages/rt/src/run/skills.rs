...
/// Scan `events.jsonl` for `skill.invoked` events; map skill → latest ts.
fn scan_invocations(project_dir: &Path, since_ms: i64) -> BTreeMap<String, String> {
    ...
    for line in raw.lines() {
        ...
        if ev.get("event").and_then(Value::as_str) != Some("skill.invoked") {
            continue;
        }
        let Some(ts) = ev.get("ts").and_then(Value::as_str) else {
            continue;
        };
        if let Some(ts_ms) = crate::run::complete_spec::parse_iso_millis(ts) {
            if ts_ms < since_ms {
                continue;
            }
        }
