...
**Why**: grain mines declaration names + domain terms straight from the code; EN-only identifiers and doc-comments keep the model's term index consistent for `scan digest` lookups. Mixed-language code terms fragment it.
