...
2. **Dispatch the validator (Sonnet).** Pass the rendered prompt VERBATIM as the Task prompt to `subagent_type: general-purpose` with `model: sonnet`. **Sonnet, not Haiku:** this is ONE routing-critical call per pipeline entry (not a fan-out), so accuracy outweighs the negligible per-call cost — a wrong `full` is the most expensive routing error. The validator replies with ONLY a JSON object:
   ...
   - `centralFound: false` means the most-discriminative concept of the intent landed under MISSED / WEAK — the kept anchors matched only common vocabulary and point at the WRONG flow. `requeryTerms` is then the real ENGLISH code identifiers that concept maps to (e.g. `efetivar → ["effectivate","settle","confirm"]`). Both default to `true` / empty when absent (an old reply carries no retrieval concern).
3. **Act on the verdict.**
   - **`centralFound: false` → RE-QUERY FIRST, before anything else.** Re-run the digest with the corrected vocabulary — `mustard-rt run feature --intent "<requeryTerms joined>"` — and use ITS deterministic anchors for the right flow; only THEN proceed to `dropped`/`route`/`scope` against the re-queried answer. A `strong` reason is **NOT** trustworthy when the central concept missed: `centralFound` OVERRIDES the aggregate `reason`. (Re-query, do not just drop — the right files were never in the first answer.) `centralFound: true` (or absent) → proceed normally.
