...
3. **Verify completed/cancelled specs before moving:**
   - If `Status: completed` or `Status: cancelled`:
     ...
     - If analysis confirms done → move from `.claude/spec/active/{name}/` to `.claude/spec/completed/{name}/`, emit `mustard-rt run emit-pipeline --kind pipeline.status --spec {name} --payload '{"status":"completed"}'` and delete `.diff.md` if it exists, log: `[HYGIENE] Verified and moved {name} → completed/`
...
Run `mustard-rt run diff-context` at the start of EXECUTE only. ANALYZE skipped (diff always empty pre-work) — emits `analyze-diff-skip` metric. Save the output to `.claude/.pipeline-states/{specName}.diff.md` (overwrite each phase; this is a local scratch file, not the canonical state source).
...
Emit to pipeline event log if Full Path:
```bash
mustard-rt run emit-pipeline --kind pipeline.root_cause --spec {specName} --payload '{
