...
1. Run `mustard-rt run event-projections --view pipeline-state --spec {newest-spec}` to find the most recent active spec and check for a recent dispatch failure event (`pipeline.dispatch_failure` kind in the SQLite event store).
2. Inspect the projected `dispatch_failure` field.
3. If present:
   - Compute `ageMs = Date.now() - new Date(dispatch_failure.at).getTime()`.
   - **If ageMs <= 10 * 60 * 1000** (≤10 min, fresh):
     ...
     2. Re-invoke the Task tool with:
        - `subagent_type`: `dispatch_failure.agentType` (fallback: `general-purpose`)
        - `description`: `dispatch_failure.description`
        - `prompt`: `dispatch_failure.prompt`
     3. After the re-dispatch returns, the `pipeline.dispatch_failure` event is superseded by the next `pipeline.status` event — no manual clear needed.
     4. Fall through to Step 1 (normal resume flow continues from the updated state).
   - **If ageMs > 10 * 60 * 1000** (stale): the projection clears automatically after >10 min — continue to Step 1.
4. If `dispatch_failure` is absent in the projection, skip Step 0 entirely and proceed to Step 0.5.
...
1. **Skip conditions — auto `reanalyze` (no prompt):**
   ...
   - `pipeline.dispatch_failure` event was present and <10min old (already handled in Step 0)
...
1b. **Auto-continue conditions — auto `continued` (no prompt):**
    - Pipeline state projection shows `updatedAt` within last 10 min AND `status === 'in_progress'` AND no `dispatch_failure` event. The user just paused or re-opened the session; trust projected state as source of truth.
...
3. **Resolve operational spec file** (the file the rest of resume operates on):
   ...
   - If matched root file is `wave-plan.md` → wave-plan mode:
     a. Run `mustard-rt run event-projections --view pipeline-state --spec {specName}`. If projection returns `isWavePlan: true` + `currentWave: N`, use that state — skip to (c).
     b. **State missing → reconstruct inline** (no `/mustard:approve` roundtrip; the user just wants to continue):
        ...
        3. **Plan already in progress** — at least one wave has `status !== "queued"` (proves it was approved & started, the state was just lost):
           - Emit reconstructed pipeline events (phase is derived from `pipeline.phase` events in SQLite; omit any phase-name field. Emit the transition via `mustard-rt run emit-phase --spec {specName} --to EXECUTE` after):
             - `mustard-rt run emit-pipeline --kind pipeline.status --spec {specName} --payload '{"status":"implementing","isWavePlan":true,"totalWaves":<N>,"completedWaves":[...],"currentWave":<M>,"reconstructedFromWavePlan":true}'`
...
5. Run `mustard-rt run event-projections --view pipeline-state --spec {specName}` (single-spec mode; wave-plan mode already loaded it in step 3) → read for current wave + scope + `explorationSummary` + `decisions`. Validate integrity (trust spec header on mismatch).
...
**Skip if `resumeMode === "continued"`** unless a wave just completed (wave transitions always refresh diff). The prior diff snapshot is reused from the local scratch file `.claude/.pipeline-states/{specName}.diff-{subproject}.md` (this is a transient scratch file, not canonical state).
...
2. Run `mustard-rt run context-slice --context {CONTEXT.md} --spec {operational_spec} > .claude/.pipeline-states/{specName}.context-md.md` (local scratch file for this session).
...
3. **Between waves** (see Step 17 post-dispatch):
   ...
   - **Cache this wave's diff:** right after the wave-{N-1} commit, run `git diff HEAD~1 HEAD > .claude/.pipeline-states/{spec-name}.wave-{N-1}.diff.md` (local scratch file — not canonical state) so the next wave can be sliced against it.
   - **Wave Slice Injection (see § Wave Slice Injection below):** before dispatching wave N (N≥2):
     ...
     2. Prepend the cached previous wave diff from the local scratch file `.claude/.pipeline-states/{spec-name}.wave-{N-1}.diff.md`. Inject `{task_steps}` + the diff (NOT the full spec, NOT the full touched files) into the agent prompt.
...
14. **Build agent prompts using template** (`.claude/refs/agent-prompt/agent-prompt.md`):
    - Read template once, then fill placeholders per agent using `.claude/pipeline-config.md` data:
      ...
      - `{context_md}` → relevance-filtered glossary slice from the local scratch file `.claude/.pipeline-states/{specName}.context-md.md` (see § Context Slice). Empty when no `CONTEXT.md` exists.
...
20. **CLOSE:**
    ...
    - Emit `mustard-rt run emit-pipeline --kind pipeline.status --spec {spec-name} --payload '{"status":"closed"}'`
...
On pause: emit `mustard-rt run emit-pipeline --kind pipeline.pause --spec {specName} --payload '{"pausedAt":"...","pauseReason":"...","nextAction":"..."}'`, then run `memory.js agent`. Handoff MUST end with exactly ONE next action (no lists of options).
