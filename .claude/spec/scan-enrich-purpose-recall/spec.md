...
- **AC-1** — Build verde.
  Command: `cargo build`
- **AC-2** — `enrich-purpose-render` emite prompt byte-estável SÓ para declarações de lógica (pula property/const/type/field), sem nenhuma chamada de modelo/rede no binário; rodar 2× dá saída idêntica.
...
- [ ] T2 — rt `enrich-purpose-render`: assembla lote determinístico (declaração de lógica → corpo) na língua do `ProjectConfig`, byte-estável, sem IO/clock/modelo; filtra kind e corpo trivial.
