...
| `sync` | `mustard-rt run sync-registry` — update `entity-registry.json` |
| `doctor` | Installation health check — wiring, drift, state + skill/OTEL diagnostics |
...
`mustard-rt run sync-registry`. Use after creating new entities, importing code, or manual edits.
...
mustard-rt run skills orphans      # skills not invoked in N days (env MUSTARD_SKILL_ORPHAN_DAYS, default 30)
