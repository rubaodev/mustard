...
/**
 * Multi-select picker for the repo model's entity names. Renders selected
 * entities as removable chips above a scrollable, searchable checkbox list. Entities
...
export interface EntityPickerProps {
  /** Full entity universe from the project's repo model (`grain.model.json`). */
  entities: string[];
  selected: string[];
  ...
  if (entities.length === 0) {
    return (
      <p className="text-xs text-muted-foreground border border-dashed border-border rounded px-3 py-2">
        Nenhuma entidade no modelo. Rode <code className="font-mono">/mustard:scan</code> no projeto para popular.
      </p>
    );
  }
