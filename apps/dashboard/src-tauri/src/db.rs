...
///
/// Delegates to [`ClaudePaths::mustard_db_path`] so the dashboard, rt, and cli
/// share one definition of where the harness DB lives. The only failure mode
/// is the I1 guard (`repo_path` would re-nest `.claude/`); in that case we
/// return an empty `PathBuf` so `cached_store`'s `db_path.exists()` check
/// short-circuits cleanly (an empty path never exists).
fn harness_db_path(repo_path: &Path) -> std::path::PathBuf {
    ClaudePaths::for_project(repo_path)
