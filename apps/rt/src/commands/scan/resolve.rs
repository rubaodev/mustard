...
//!    [`crate::hooks::task::context_budget_gate::role_prompt_budget`]). The farthest-distance
...
pub struct ResolveScope {
    ...
    /// the budget cut via [`crate::hooks::task::context_budget_gate::role_prompt_budget`].
    #[serde(default)]
...
pub fn resolve_closure(project_root: &Path, scope: &ResolveScope) -> ResolveOutput {
    ...
    let budget = scope
        .role
        .as_deref()
        .and_then(crate::hooks::task::context_budget_gate::role_prompt_budget);
