...
- **RESOLUÇÃO DOS PENDENTES (2026-06-25, validação honesta):**
  ...
  - ⏳ **3º projeto / 30 casos (hoje 22/2):** receita PROVADA e barata (Medusa: `git clone --depth 1` + `scan --root <módulo>` + 5 agentes Sonnet enrich PT cegos em paralelo + 1 Explore rotulador + `recall-bench` = ~$1, ~3min). Repetir num 3º repo (idealmente OUTRA linguagem p/ provar poliglota — ex.: Saleor/Python) fecha 30/3.
  - ⏳ **Resíduo de sinônimo** (estornar→delete, separar/expedição→fulfillment): classe conhecida → ponte de léxico PT (estornar↔reverter) no `.claude/lexicons` do projeto; o ladder já tem tier lexicon. Demo pendente.
