...
| `{acceptance_block}` | the WAVE's own `## Acceptance Criteria` (materialised by the wave scaffold from the ids that wave `satisfies`), verbatim — `Command:` / `Expect:` / `Control:` included | VARIABLE — the ruler the wave is judged by. The union stays in `wave-plan.md`, which is what QA executes; this is the subset that wave claims. Empty when the wave declares none (and the plan is REFUSED when such a wave carries tasks). |
